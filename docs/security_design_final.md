...
This document describes the security design of CypherShield, an application that allows users to register, upload encrypted documents, share them with specific users under controlled roles, and maintain a full audit trail of all actions.

**Security goals:**
- Protect document confidentiality at rest and in transit.
...
**Architecture.** The system is a Python/Flask server-rendered web application using file-based JSON for data storage. Sessions are managed entirely server-side with `secrets.token_urlsafe(32)` tokens. Passwords are hashed with bcrypt (cost factor 12). HTTP security headers are applied to every response.

**Threat model.** Seventeen threats are identified using the STRIDE framework, covering spoofing, tampering, information disclosure, denial of service, and privilege escalation. Each threat is mapped to implemented mitigations and residual risks.

**Testing.** The automated test suite includes 78 passing tests covering registration, login, lockout, rate limiting, RBAC, document upload/download/share/delete, session lifecycle, and audit logging.

**Known limitations.** Residual risks include EICAR-only malware scanning, single-key encryption without rotation, JSON file storage concurrency constraints, single-factor password resets, and CSP `'unsafe-inline'`. Each limitation is documented with a proposed future improvement.
...
CypherShield is a web application that enables users to upload, encrypt, share, and collaboratively manage confidential documents under strict access controls. The system is designed to protect document confidentiality at rest and in transit, enforce role-based and document-level authorization, maintain a complete audit trail of all security-relevant events, and resist common web application attacks.

The primary assets protected by the system are:

- **Uploaded documents** — encrypted at rest using symmetric encryption (Fernet) before being written to disk.
- **User credentials** — passwords are hashed with bcrypt (cost factor 12) to avoid storage or transmission of plaintext.
...
The system follows a traditional server-rendered web application architecture with three logical tiers:
...
│  — Receives server-rendered HTML responses                      │
...
│                                                                 │
│  ┌──────────────┐  ┌──────────────┐  ┌───────────────────────┐  │
│  │   auth.py    │  │ sessions.py  │  │  access_control.py    │  │
│  │ Registration │  │  Create/Get/ │  │  RBAC + Document      │  │
│  │ Login/Lockout│  │  Invalidate  │  │  Role enforcement     │  │
│  └──────────────┘  └──────────────┘  └───────────────────────┘  │
│  ┌──────────────┐  ┌──────────────┐  ┌───────────────────────┐  │
│  │ documents.py │  │ security.py  │  │  logging_utils.py     │  │
│  │ Upload/Share │  │ Input valid. │  │  SecurityLogger &     │  │
│  │ Encrypt/Vers │  │ Sec headers  │  │  AccessLogger         │  │
│  └──────────────┘  └──────────────┘  └───────────────────────┘  │
│  ┌──────────────┐  ┌──────────────┐                             │
│  │  storage.py  │  │  config.py   │                             │
│  │  JSON R/W    │  │  Central conf│                             │
│  │  Atomic write│  │  Env-driven  │                             │
│  └──────────────┘  └──────────────┘                             │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
...
│                                                                 │
│  data/  — users.json, sessions.json, documents.json,            │
│           shares.json, audit.json, rate_limits.json,            │
│           documents/*.bin (encrypted payloads)                  │
│  logs/  — security.log, access.log                              │
...
3. **CSRF validation** (`enforce_csrf`) — For all state-changing HTTP methods (POST, PUT, PATCH, DELETE), the server verifies that the submitted `csrf_token` form field matches the token stored in the server-side session. Mismatches are logged and rejected.
...
The application logic is organized into the following backend modules:
...
|---|---|---|
| **Python 3 / Flask** | Web framework | Minimalist design provides explicit control over every security middleware, strengthening auditability. |
| **bcrypt (cost factor 12)** | Password hashing | Cost factor 12 is the specified minimum. Built-in salt and adaptive cost resist GPU brute-force. |
| **cryptography / Fernet** | Data-at-rest encryption | Provides authenticated symmetric encryption (AES-128-CBC + HMAC-SHA256); tampering detected at decryption. |
| **JSON file storage** | Persistence | Self-contained; trade-off is no concurrent writes (acceptable for teaching scope). |
| **Server-side sessions** | Session management | Sessions stored in `sessions.json` (not client-side cookies). Enables single-session-per-user, explicit invalidation, and sliding-window timeouts. |
| **Jinja2 auto-escaping** | XSS prevention | Auto-escapes all template variables. Application additionally uses `html.escape()` in `security.py` for non-template contexts. |
| **Python `secrets` module** | Token generation | `secrets.token_urlsafe(32)` draws from OS CSPRNG for both session and CSRF tokens. |

---

## 3. Data Flow

This section traces the four primary data flows through the system and where each security control is applied.

### 3.1 User Registration and Authentication

**Registration flow:**

1. Browser → `POST /register` (username, email, password, confirm)
2. `validate_username()` → `validate_email()` → `validate_password_strength()`
3. Check duplicate username/email in `users.json`
4. `bcrypt.hashpw(cost=12)` → save to `users.json` → log `USER_REGISTERED`
5. Invalidate prior sessions → `create_session()` (token + CSRF)
6. Write `sessions.json` → log `SESSION_CREATED`
7. Response: 302 + Set-Cookie (HttpOnly, Secure, SameSite=Strict)
...
- **Automatic session creation**: A new server-side session is created immediately after registration.
- **Event logging**: `USER_REGISTERED` and `SESSION_CREATED` events are logged with IP and timestamp.

**Login flow:**

1. Browser → `POST /login` (identifier, password)
2. `_check_rate_limit(IP)` — at most 10 per IP per minute
3. Find user by name or email → check `locked_until`
4. `bcrypt.checkpw()` — on success: reset `failed_attempts`, log `LOGIN_SUCCESS`
5. On failure: increment `failed_attempts`; if ≥ 5 → lock 15 min, log `LOGIN_FAILED`/`ACCOUNT_LOCKED`
6. `invalidate_user_sessions()` → `create_session()` → log `SESSION_CREATED`
7. Response: 302 + Set-Cookie

Key security controls:
- **Per-IP rate limiting**: At most 10 login attempts per IP address per minute, tracked in `rate_limits.json`. Exceeding the limit logs a `SUSPICIOUS_ACTIVITY` event.
...
1. Browser → `POST /upload` (title, type, file)
2. `require_auth` + `require_role(can_create_content)` — authorization gate
3. `validate_document_title()` → `validate_uploaded_file()`:
   - Extension whitelist → MIME type check → magic-byte signature → EICAR malware scan
4. `Fernet.encrypt(plaintext)` → write encrypted `.bin` to disk
5. SHA-256 hash of plaintext stored in metadata
6. Append to `documents.json` → append audit event → log `DOCUMENT_UPLOAD`
...
1. Browser → `POST /share` (recipient, access_role)
2. `require_auth` → `authorize_document_access()`:
   - `get_document_role()` from `shares.json`
   - `authorize_document_share_management()` — owner or admin only
3. `share_document_with_user()`:
   - Validate role in {viewer, editor} → check not self-share → check recipient exists
4. Upsert `shares.json` → log `DOCUMENT_SHARED` → append audit event
5. Response: 200 success
...
1. Browser → `GET /download/<document_id>`
2. `require_auth` → `authorize_document_download()` (system role + document role)
3. `get_document_revision()` — resolve requested version from history
4. `safe_file_path()` — path traversal check
5. Read encrypted `.bin` → `Fernet.decrypt()` (AES + HMAC verify)
6. Log `DOCUMENT_DOWNLOAD` → append audit event
7. Response: 200 binary (Content-Disposition: attachment)
...
The following table classifies the system's protected assets by sensitivity and maps each to their storage locations and primary defense mechanisms:
...
| T17 | Spoofing | Account hijacking via password change without secondary verification | `auth.py` — `change_password()` endpoint |
...
| T17 | Password change requires current password and active session; all `PASSWORD_CHANGE` events are audit-logged with IP and timestamp | No secondary verification (email confirmation or MFA) is required — an attacker who obtains a user's current password can immediately change it via the change-password form, permanently hijacking the account and locking out the original user |

### 4.4 Attack Scenarios

**Scenario 1 — Password Reset Attack.**
An attacker gains knowledge of an authenticated user's password. The attacker could immediately change the user's password via the change-password form. This results in the hijacking of the original user's account, permanently locking them out from their account and associated documents.
...
An attacker uploads a file named `report.pdf` that is actually a script. The magic-byte signature check detects that the file content does not start with `%PDF-`, and the upload is rejected. The malware scan also checks for the EICAR test signature. If found, both failures are logged as `UPLOAD_VALIDATION_FAILED`.

**Scenario 5 — Path traversal via document download.**
An attacker crafts a request with a manipulated document ID intending to traverse to `../../etc/passwd`. The `safe_file_path()` function calls `secure_filename()` to strip path separators, then resolves the absolute path and verifies it falls within `DOCUMENT_STORAGE_DIR`. The traversal attempt fails, and no file outside the storage directory is served.
...
| **High** | T1, T2, T3, T15, T16, T17 | Credential theft or privilege escalation enabling further attacks |
...
**System RBAC.** Three roles defined in `access_control.py` with the following permission matrix:
...
| Manage users | Yes | -- | -- |
| View shared content | Yes | Yes | Yes |

`require_role()` checks permissions before route logic; failures log `AUTHORIZATION_DENIED` (403). The following are tested in `test_access_control.py`: admin panel access, user 403 on admin, guest upload blocked, role change audit-logged. Limitations: Admin credentials are publicly available in README for grading purposes; admin credentials in real deployments would be closely guarded.

**Document role authorization.** Three document roles: `owner`, `editor`, `viewer`. `get_document_role()` resolves from `shares.json`; owner assigned to document's creator. Permission enforcement: view/preview (any role), edit (owner/editor), delete (owner only), manage shares (owner/admin), download (any role). Editor-to-owner return functionality downgrades the editor to viewer. Failures log `DOCUMENT_ACCESS_DENIED`, `DOCUMENT_DOWNLOAD_DENIED`, `DOCUMENT_DELETE_DENIED`, or `DOCUMENT_SHARE_DENIED`. The following is tested in `test_documents.py` and `test_access_control.py`: owner downloads, viewer downloads, unshared user 403, return-to-owner access downgrade. Limitations: Many simultaneous editors can create access issues and version control inconsistencies.
...
`apply_security_headers()` in `security.py` is called via `@app.after_request` on every response:
...
Tested with `securityheaders.com`. Limitation: `'unsafe-inline'` in CSP weakens XSS protection; real-world deployments would adopt CSP nonces.

### 5.7 Logging and Monitoring

Two dedicated loggers in `logging_utils.py`: **SecurityLogger** (`logs/security.log`) and **AccessLogger** (`logs/access.log`). Each entry is structured JSON with `timestamp`, `event_type`, `user_id`, `ip_address`, `user_agent`, `details`, and `severity`. Document events are also appended to `data/audit.json` for the admin dashboard.
...
**Current test results:** 78 tests passed (0 failures, 0 errors).
...
In addition to automated tests, the following manual black-box tests have been performed:
...
The following table maps each security requirement to the test evidence that validates it:
...
| L11 | Password reset | The application supports authenticated password change (requires current password) but does not implement a "forgot password" reset flow with email verification or multi-factor authentication | An attacker who obtains a user's password could change it immediately via the change-password form, hijacking the account and permanently locking out the original user |

### 8.2 Proposed Future Improvements

| Priority | Improvement | Addresses |
|---|---|---|
| High | Integrate a third-party antivirus API for real, extensive malware detection | L1 |
...
| Medium | Implement secure password reset via time-limited email token with MFA verification; require email confirmation before allowing password change to prevent account hijacking | L11 |
